...
# 升级点 1：经过“瘦身”的极简黄金特征矩阵
# 砍掉所有衍生特征，防止深度网络过拟合，让网络自己去发掘交叉关系
# 注意：请确保你的 build_features_pipeline 会输出 'HDD' 这个基础列
SELECTED_FEATURES = [
    "Lag_12", 
    "HDD", 
    "is_heating_season"
...
class GasForecastGRU(nn.Module):
    ...
    def forward(self, x_cont: torch.Tensor, x_month: torch.Tensor) -> torch.Tensor:
        # x_month 的 shape: (batch_size, seq_length), dtype=torch.long
        # 经过 Embedding 后变成: (batch_size, seq_length, emb_dim)
        month_vecs = self.month_emb(x_month)
        
        # 升级点 3：将业务气象特征与自学习的时间向量进行全维度拼接
        # x 最终 shape: (batch_size, seq_length, total_input_size)
...
def _create_3d_sequences(
    ...
    for i in range(len(x_cont_arr) - seq_length):
        ...
        xs_month.append(x_month_arr[i : i + seq_length]) # 滑动截取月份序列
...
def main() -> None:
    ...
    if missing_features:
        raise ValueError(f"特征工程后缺少所需特征: {missing_features}。请检查 pipeline 是否生成了这些特征。")

    # 分离出连续特征、月份离散特征、目标值和时间轴
    month_arr = df_features[MONTH_COL].astype(str).to_numpy()
    x_cont_all = df_features[SELECTED_FEATURES].to_numpy(dtype=float)
    x_month_all = df_features['month_idx'].to_numpy(dtype=int) # 1-12 的整数
    ...
    # 标准化：只对连续特征(气象/销量)标准化，绝对不能对分类的月份整数标准化
    ...
    # 转换为 PyTorch 张量 (月份必须是 torch.long 整型才能过 Embedding)
    ...
    excel_path = os.path.join(
        OUTPUT_DIR,
        f"{PROCESSED_PROVINCE}_gru_test_{TEST_START}_to_{TEST_END}.xlsx",
    )
    ...
    if np.isfinite(mape):
        print(f"\nGRU 最终测试集 ({TEST_START} ~ {TEST_END}) 成绩:")
