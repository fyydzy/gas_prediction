...
# XGBoost 残差模型输入特征：与 run_xgboost_0.py 一致，使用 build_features_pipeline 生成的全部特征
...
TIMESFM_FREQ_MONTHLY = 2
TIMESFM_INSAMPLE_MIN_HISTORY = 24
...
def _load_series_with_weather(path: str) -> pd.DataFrame:
    ...
    # 与 run_xgboost_0.py 对齐：走完整特征工程流水线（含 HDD / lag / 交互 / 时间周期等）
    return build_features_pipeline(out, target_col=TARGET_COL, month_col=MONTH_COL)


def _build_timesfm(horizon_len: int) -> timesfm.TimesFm:
...
def _pad_for_timesfm(values: np.ndarray) -> np.ndarray:
    x = np.asarray(values, dtype=np.float32)
    pad = (-len(x)) % TIMESFM_PATCH_LEN
    if pad:
        x = np.concatenate([np.full(pad, np.nan, dtype=np.float32), x])
    return x


def _forecast_monthly(train_values: np.ndarray, horizon_len: int) -> np.ndarray:
    model = _build_timesfm(horizon_len=horizon_len)
    pred = model.forecast([_pad_for_timesfm(train_values)], freq=[TIMESFM_FREQ_MONTHLY])
...
def _insample_one_step_predictions(context_values: np.ndarray) -> np.ndarray:
    """
    为残差学习构造 TimesFM 的历史预测值：
    - 前 TIMESFM_INSAMPLE_MIN_HISTORY 个月直接用真实值占位（残差=0）
    - 之后对每个时点用其历史前缀做 1-step 预测（滚动回测）
    """
    y = np.asarray(context_values, dtype=np.float32)
    n = len(y)
    if n == 0:
        return y

    preds = y.copy()
    start = min(TIMESFM_INSAMPLE_MIN_HISTORY, n)
    if start >= n:
        return preds

    prefixes = [_pad_for_timesfm(y[:i]) for i in range(start, n)]
    model = _build_timesfm(horizon_len=1)
    raw = model.forecast(prefixes, freq=[TIMESFM_FREQ_MONTHLY] * len(prefixes))
    pred_arr = np.asarray(raw[0] if isinstance(raw, tuple) else raw)
    if pred_arr.ndim == 2:
        one_step = pred_arr[:, 0]
    else:
        one_step = pred_arr
    preds[start:] = one_step.astype(np.float32)
    return preds
...
def _train_xgb_residual_model(
    ...
    if len(context_df) != len(context_base_pred):
        raise ValueError("context 长度与 TimesFM 历史拟合长度不一致。")

    residual_df = context_df.copy()
    residual_df["timesfm_yhat_in_sample"] = context_base_pred.astype(float)
    residual_df["residual"] = residual_df[TARGET_COL].to_numpy(dtype=float) - residual_df[
        "timesfm_yhat_in_sample"
...
def main() -> None:
    ...
    for context_months in candidate_contexts:
        ...
        context_values = context_df[TARGET_COL].to_numpy(dtype=float)
        base_forecast = _forecast_monthly(context_values, horizon_len=total_horizon).astype(float)
        context_base_pred = _insample_one_step_predictions(context_values).astype(float)
        ...
        chosen_label = "timesfm_plus_xgb_residual" if use_hybrid else "timesfm_only"
        chosen_mape = (
    ...
    out_path = os.path.join(
        OUTPUT_DIR,
        f"{PROCESSED_PROVINCE}_timesfm_xgbres_ctx{ctx_suffix}_{INFERENCE_CONTEXT_END}_{total_horizon}m.xlsx",
    )
    ...
    print(
        f"  TimesFM基线 | MAE={metrics_base['MAE']:.6f} RMSE={metrics_base['RMSE']:.6f} "
        f"MAPE={metrics_base['MAPE(%)']:.6f}%"
    )
    print(
        f"  TimesFM+XGB残差 | MAE={metrics_hybrid['MAE']:.6f} RMSE={metrics_hybrid['RMSE']:.6f} "
