...
OUTPUT_DIR = "output1"
# XGBoost 残差特征：build_features_pipeline 生成后，除 date / gas_sales 外的全部数值特征列（运行时确定）
...
def _train_xgb_residual_model(
    ...
    n = len(residual_df)
    if n < 36:
        raise ValueError("context 旬度过少，无法稳定训练 XGBoost 残差模型（至少建议 36 旬）。")
    ...
    if XGB_VERBOSE_FIT:
        print(
            f"XGBoost 残差训练: 训练 {x_train.shape[0]} 旬 | 验证 {x_val.shape[0]} 旬（时间顺序末尾）| "
...
def main() -> None:
    ...
    print(f"XGBoost 残差特征（来自 feature_engineering1）: {RESIDUAL_FEATURES}")
    ...
    metrics_rows = pd.DataFrame(
        [
            ...
            {"model": "prophet_plus_xgb_residual_test", **metrics_hybrid},
    ...
    out_path = os.path.join(
        OUTPUT_DIR,
        f"{PROCESSED_PROVINCE}_prophet_xgbres_ctx{ctx_suffix}_{INFERENCE_CONTEXT_END}_{total_horizon}t.xlsx",
    )
    ...
    if isinstance(best_xgb_model, XGBRegressor):
        ...
        _save_xgb_feature_importance_plot(
            ...
            output_path=fi_path,
            title=f"{PROCESSED_PROVINCE} Prophet+XGB 残差特征重要性（旬度）",
        )
    ...
    print("  Prophet+XGB 残差:", metrics_hybrid)
